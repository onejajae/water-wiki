...
for product_name, product in products.items():
    print(product)
    current_vendor = (
        db_session.query(Vendor).filter(Vendor.name == product["vendor"]).first()
    )
    ...
    product_in = Product(name=product_name, vendor_id=current_vendor_id)
    db_session.add(product_in)

    if current_vendor:
        current_vendor.products.append(product_in)

    for supplier_name in product["suppliers"]:
        current_supplier = (
            db_session.query(Supplier).filter(Supplier.name == supplier_name).first()
        )
